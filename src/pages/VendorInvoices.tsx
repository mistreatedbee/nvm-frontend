...
export function VendorInvoices() {
  ...
  const loadInvoices = async () => {
    try {
      const response = await invoicesAPI.getVendorInvoices({ limit: 50 });
      setInvoices(response.data?.data || []);
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to load vendor invoices');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = async (invoiceId: string, invoiceNumber: string) => {
    try {
      const response = await invoicesAPI.downloadVendorPdf(invoiceId);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-nvm-dark-900 mb-6">Vendor Invoices</h1>
        {loading ? (
