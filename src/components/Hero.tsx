...
export function Hero() {
  ...
  const fetchStats = async () => {
    try {
      // #region agent log
      // Send debug telemetry to a local ingest agent only when explicitly enabled.
      if (String(import.meta.env.VITE_ENABLE_LOCAL_AGENT || '').toLowerCase() === 'true') {
        fetch('http://127.0.0.1:7469/ingest/5d631cb0-b818-47aa-aa02-aaea8bc641b1', {
          ...
          body: JSON.stringify({
            sessionId: '7840a6',
            location: 'Hero.tsx:fetchStats',
            message: 'Hero firing 3 parallel requests',
            data: { endpoints: ['vendors', 'products', 'products?status=PUBLISHED'] },
            timestamp: Date.now(),
            hypothesisId: 'H2'
          })
        }).catch(() => {});
